...
public class CardExchangeView implements Observer {
    ...
    public void update(Observable o, Object arg) {
    	...
    	//  If the player selects cards, they are given the appropriate number of armies as reinforcement. The player can choose
    	//  not to exchange cards and exit the card exchange view. If the player own 5 cards or more, they must exchange cards. The cards
    		...
    		while (true) {
	    		...
	    		case Commands.MAP_COMMAND_REINFORCE_OPTION_EXCHANGECARDS:
	    			...
	    			if (words[1].equalsIgnoreCase("-none")) {
	    	    		System.out.println(gameController.getCurrentPlayer() + 
	    	    				" has chosen not to exchange cards");
	    	    		return;
	    			}
					...
					if (retVal) {
						gameController.getCardModel().exchangeCards(gameController.getCurrentPlayer(),
								idx, cardsChoosen, gameController.getCardsStack());
						return;
					}
					else
						System.out.println("You can only exchange when \n1.Cards of all same type or \n2.Cards of all different type");
					break;
